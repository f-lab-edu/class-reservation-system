...
class AcademyController(
    ...
    fun registerAcademy(@RequestBody academyReqDto: AcademyReqDto): ResponseEntity<*> {
        return ResponseEntity.ok(ResponseDto(200, "Success", academyUseCase.registerAcademy(academyReqDto)))
    }

    @PostMapping("/searching")
    fun searchingAcademy(@PathVariable academyId: Int): ResponseEntity<*> {
        return ResponseEntity.ok(ResponseDto(200, "Success", academyUseCase.searchingAcademy(academyId)))
    }
}